...
dotnet restore "Source/PMTribal/PMTribal.sln"
...
."C:\Program Files (x86)\Microsoft Visual Studio\$VSVersion\Community\MSBuild\Current\Bin\MSBuild.exe"  "Source/PMTribal/PMTribal.sln" /t:restore
...
."C:\Program Files (x86)\Microsoft Visual Studio\$VSVersion\Community\MSBuild\Current\Bin\MSBuild.exe"  "Source/PMTribal/PMTribal.sln" /t:Rebuild /p:Configuration=Debug /p:Platform="any cpu"
...
Copy-Item -Path Defs, About, Languages, Assemblies, Patches, Source, Textures -Destination "$buildDir/Tmp" -Recurse
...
if(Test-Path "$buildDir/Tmp/Source/PMTribal/.vs")
{
    Remove-Item "$buildDir/Tmp/Source/PMTribal/.vs" -Recurse -Force
}
#get rid of nuget packages

if(Test-Path "$buildDir/Tmp/Source/PMTribal/packages")
{
    Remove-Item "$buildDir/Tmp/Source/PMTribal/packages" -Force -Recurse
}
